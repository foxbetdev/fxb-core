...
public:
    ...
    bool WriteZFXBSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZFXBSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZFXBSeed_deprecated(uint256& seed);
    bool EraseZFXBSeed();
    bool EraseZFXBSeed_deprecated();

    bool WriteZFXBCount(const uint32_t& nCount);
    bool ReadZFXBCount(uint32_t& nCount);
