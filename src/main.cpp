...
#if defined(NDEBUG)
#error "FXB cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ufxb) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zFXB spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zFXB is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend.HasValidSignature())
                return error("%s: V2 zFXB spend does not have a valid signature\n", __func__);
        ...
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zFXB without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (!spend.HasValidSerial(Params().Zerocoin_Params(fUseV1Params))) {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zFXB spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zFXB spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zfxb");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zFXB mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
         nSubsidy = 230000 * COIN;
    } else if (nHeight <= 200 && nHeight > 0) {
         nSubsidy = 0 * COIN;
    } else if (nHeight <= 4230 && nHeight > 200) {
         nSubsidy = 0.3 * COIN;
    } else if (nHeight <= 14310 && nHeight > 4230) {
         nSubsidy = 0.8 * COIN;
    } else if (nHeight <= 24390 && nHeight > 14310) {
         nSubsidy = 1.5 * COIN;
    } else if (nHeight <= 34470 && nHeight > 24390) {
         nSubsidy = 2 * COIN;
    } else if (nHeight <= 44550 && nHeight > 34470) {
         nSubsidy = 2.5 * COIN;
    } else if (nHeight <= 54630 && nHeight > 44550) {
         nSubsidy = 3 * COIN;
    } else if (nHeight <= 64710 && nHeight > 54630) {
         nSubsidy = 3.5 * COIN;
    } else if (nHeight <= 74790 && nHeight > 64710) {
         nSubsidy = 4 * COIN;
    } else if (nHeight <= 84870 && nHeight > 74790) {
         nSubsidy = 4.5 * COIN;
    } else if (nHeight <= 94950 && nHeight > 84870) {
         nSubsidy = 5 * COIN;
    } else if (nHeight <= 105030 && nHeight > 94950) {
         nSubsidy = 5.5 * COIN;
    } else if (nHeight <= 115110 && nHeight > 105030) {
         nSubsidy = 6 * COIN;
    } else if (nHeight <= 125190 && nHeight > 115110) {
         nSubsidy = 6.5 * COIN;
    } else if (nHeight <= 135270 && nHeight > 125190) {
         nSubsidy = 7 * COIN;
    } else if (nHeight <= 145350 && nHeight > 135270) {
         nSubsidy = 7.5 * COIN;
    } else if (nHeight <= 155430 && nHeight > 145350) {
         nSubsidy = 8 * COIN;
    } else if (nHeight <= 165510 && nHeight > 155430) {
         nSubsidy = 8.5 * COIN;
    } else if (nHeight <= 175590 && nHeight > 165510) {
         nSubsidy = 9 * COIN;
    } else if (nHeight <= 185670 && nHeight > 175590) {
         nSubsidy = 9.5 * COIN;
    } else if (nHeight <= 228870 && nHeight > 185670) {
         nSubsidy = 10 * COIN;
    } else if (nHeight <= 272070 && nHeight > 228870) {
         nSubsidy = 8 * COIN;
    } else if (nHeight <= 315270 && nHeight > 272070) {
         nSubsidy = 6 * COIN;
    } else if (nHeight > 315270) {
         nSubsidy = 4 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZFXBStake)
{
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from FXB
...
void RecalculateZFXBMinted()
{
    ...
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZFXB..."), percent);
        }
...
void RecalculateZFXBSpent()
{
    ...
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = (int)( (double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()) );
            uiInterface.ShowProgress(_("Recalculating spent ZFXB..."), percent);
        }
        ...
        //Remove spends from zFXB supply
...
bool RecalculateFXBSupply(int nHeightStart)
{
    ...
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating FXB supply..."), percent);
        }
...
bool UpdateZFXBSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zFXB supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateFXBSupply(Params().Zerocoin_StartHeight());
    }

    //Track zFXB money supply in the block index
    if (!UpdateZFXBSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zFXB supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zFxbSpent: %s\n",
...
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert FXB to zFXB
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zFXB spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zFXB serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zFXB stake block is less than allowed start height", __func__);

    if (CZFxbStake* zFXB = dynamic_cast<CZFxbStake*>(stake)) {
        CBlockIndex* pindexFrom = zFXB->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zFXB stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zFXB stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zFXB->GetValue()));
        if (nChecksum200 != zFXB->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zFXB->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZFXB() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zFXB fails context checks", __func__));
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> fxbInputs;
        std::vector<CTxIn> zFXBInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.scriptSig.IsZerocoinSpend()){
                zFXBInputs.push_back(stakeIn);
            }else{
                fxbInputs.push_back(stakeIn);
            }
        }
        const bool hasFXBInputs = !fxbInputs.empty();
        const bool hasZFXBInputs = !zFXBInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasFXBInputs)
                    ...
                    for (const CTxIn& fxbIn : fxbInputs){
                        if(fxbIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    for (const CTxIn& in: t.vin) {
                        ...
                        for (const CTxIn& stakeIn : fxbInputs) {
            ...
            // Now that this loop if completed. Check if we have zFXB inputs.
            if(hasZFXBInputs){
                for (const CTxIn& zFxbInput : zFXBInputs) {
                    CoinSpend spend = TxInToZerocoinSpend(zFxbInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zfxb");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zFXB mints and %d zFXB spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // FXB: We use certain sporks during IBD, so check to see if they are
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zfxb", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zfxb", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
