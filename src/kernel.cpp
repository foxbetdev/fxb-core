...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZFxbStake(spend));
    } else {
        ...
        CFxbStake* fxbInput = new CFxbStake();
        fxbInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(fxbInput);
    }
