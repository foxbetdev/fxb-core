...
#include "ui_zfxbcontroldialog.h"
...
std::set<std::string> ZFxbControlDialog::setSelectedMints;
std::set<CMintMeta> ZFxbControlDialog::setMints;

bool CZFxbControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZFxbControlDialog::COLUMN_DENOMINATION || column == ZFxbControlDialog::COLUMN_VERSION || column == ZFxbControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZFxbControlDialog::ZFxbControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZFxbControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZFxbControlDialog::~ZFxbControlDialog()
{
    delete ui;
}

void ZFxbControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZFxbControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZFxbControlWidgetItem* itemDenom(new CZFxbControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZFxbControlWidgetItem *itemMint = new CZFxbControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zfxbTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zfxbTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zFXB.";
            else if (!mint.isSeedCorrect)
                strReason = "The zFXB seed used to mint this zFXB is not the same as currently hold in the wallet";
            else
...
void ZFxbControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZFxbControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZFxb_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZFxbControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZFxbControlDialog::GetSelectedMints()
{
...
void ZFxbControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
