                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zFXB ought to be enough for anybody." - Bill Gates, 2017
    ui->zFXBpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzFXBSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zFXBpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzFXB_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zFXB is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zFXB, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zFXB...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zFXB in ") +
...
void PrivacyDialog::on_pushButtonSpendzFXB_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zFXB is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zFXB, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zFXB
        sendzFXB();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zFXB
    sendzFXB();
}

void PrivacyDialog::on_pushButtonZFxbControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZFxbControlDialog* zFxbControl = new ZFxbControlDialog(this);
    zFxbControl->setModel(walletModel);
    zFxbControl->exec();
}

void PrivacyDialog::setZFxbControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzFxbSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzFXB()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Fxb Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zFXBpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zFXB is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " FXB </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zFXB</b>";
    ...
    // use mints from zFXB selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZFxbControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZFxbControlDialog::GetSelectedMints();
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zFXB was spent successfully update the addressbook with the label
    ...
    // Clear zfxb selector in case it was used
    ZFxbControlDialog::setSelectedMints.clear();
    ui->labelzFxbSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zFXB Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Fxb, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zFXB Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zfxbTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zFXB </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zFXB "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zFXB "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zFXB "));
    ui->labelzFXBAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zFXB </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zFXB </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zFXB </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzFXBSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzFXB->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zFXB
        ui->pushButtonMintzFXB->setEnabled(false);
        ui->pushButtonMintzFXB->setToolTip(tr("zFXB is currently disabled due to maintenance."));

        // Spend zFXB
        ui->pushButtonSpendzFXB->setEnabled(false);
        ui->pushButtonSpendzFXB->setToolTip(tr("zFXB is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zFXB
        ui->pushButtonMintzFXB->setEnabled(true);
        ui->pushButtonMintzFXB->setToolTip(tr("PrivacyDialog", "Enter an amount of FXB to convert to zFXB", 0));

        // Spend zFXB
        ui->pushButtonSpendzFXB->setEnabled(true);
        ui->pushButtonSpendzFXB->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
