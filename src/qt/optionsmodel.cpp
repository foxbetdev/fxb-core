...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::FXB);
    ...
    if (!settings.contains("nAnonymizeFxbAmount"))
        settings.setValue("nAnonymizeFxbAmount", 1000);

    nAnonymizeFxbAmount = settings.value("nAnonymizeFxbAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeFxbAmount"))
        SoftSetArg("-anonymizefxbamount", settings.value("nAnonymizeFxbAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in fxb.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeFxbAmount:
            return QVariant(nAnonymizeFxbAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeFxbAmount:
            nAnonymizeFxbAmount = value.toInt();
            settings.setValue("nAnonymizeFxbAmount", nAnonymizeFxbAmount);
            emit anonymizeFxbAmountChanged(nAnonymizeFxbAmount);
            break;
        case CoinControlFeatures:
