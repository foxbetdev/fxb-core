...
CzFXBTracker::CzFXBTracker(std::string strWalletFile)
{
...
CzFXBTracker::~CzFXBTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzFXBTracker::Init()
{
...
bool CzFXBTracker::Archive(CMintMeta& meta)
{
...
bool CzFXBTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzFXBTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzFXBTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzFXBTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzFXBTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzFXBTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzFXBTracker::GetSerialHashes()
{
...
CAmount CzFXBTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzFXBTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzFXBTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzFXBTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzFXBTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzFXBTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzFXBTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzFXBTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzFXBTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzFXBTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzFXBTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzFXBWallet* zFXBWallet)
{
    bool iszFXBWalletInitialized = (NULL != zFXBWallet);
    CMintMeta meta;
    ...
    if (! iszFXBWalletInitialized)
        zFXBWallet = new CzFXBWallet(strWalletFile);
    meta.isSeedCorrect = zFXBWallet->CheckSeed(dMint);
    if (! iszFXBWalletInitialized)
        delete zFXBWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzFXBTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzFXBTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzFXBTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzFXBTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzFXBTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzFXBTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzFXBWallet* zFXBWallet = new CzFXBWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zFXBWallet);
        }
        delete zFXBWallet;
        LogPrint("zero", "%s: added %d dzfxb from DB\n", __func__, listDeterministicDB.size());
    }
