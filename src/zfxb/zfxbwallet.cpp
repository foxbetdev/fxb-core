...
CzFXBWallet::CzFXBWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zfxb seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZFXBSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZFXBSeed_deprecated()) {
                    LogPrintf("%s: Updated zFXB seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zfxb seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zfxb wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzFXBWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZFXBCount(nCountLastUsed);
    else if (!walletdb.ReadZFXBCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzFXBWallet::Lock()
{
    seedMaster = 0;
}

void CzFXBWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzFXBWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZFXB(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzFXBWallet::LoadMintPoolFromDB()
{
...
void CzFXBWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzFXBWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzFXBWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zfxbTracker->HasPubcoinHash(pMint.first)) {
...
bool CzFXBWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZFXB(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zfxbTracker which also adds to database
    pwalletMain->zfxbTracker->Add(dMint, true);
...
void CzFXBWallet::SeedToZFXB(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzFXBWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzFXBWallet::GenerateDeterministicZFXB(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzFXBWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzFXBWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzFXBWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
